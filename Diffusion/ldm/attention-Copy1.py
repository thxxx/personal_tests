...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        Q = rearrange(Q, 'b (h d) x y -> b h (x y) d', h=self.num_heads) #아 이게 flatten이구나??? 맞네 그 생각을 못했네..
...
class DiT(nn.Module):
    def __init__(self, in_dim, model_dim, resolution, depth, num_heads):
        ...
        self.to_out = nn.Conv2d(model_dim, in_dim, kernel_size=1, stride=1, padding=0)
