...
def ddim_sample(x_t, t, predicted_noise, alphas_bar_t, alphas_bar_t_1):
    # 모든 인자는 shape (B, ...)
    sqrt_ab_t = alphas_bar_t.sqrt().view(-1, 1, 1)
    sqrt_ab_t_1 = alphas_bar_t_1.sqrt().view(-1, 1, 1)
    sqrt_one_minus_ab_t = (1 - alphas_bar_t).sqrt().view(-1, 1, 1)
    sqrt_one_minus_ab_t_1 = (1 - alphas_bar_t_1).sqrt().view(-1, 1, 1)

    x0_pred = (x_t - sqrt_one_minus_ab_t * predicted_noise) / sqrt_ab_t
    x_prev = sqrt_ab_t_1 * x0_pred + sqrt_one_minus_ab_t_1 * predicted_noise
    return x_prev
...
def x_t_sample(x_0, timesteps, noise, alphas_bar):
