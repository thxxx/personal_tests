...
from utils import visualize, show_tensor_image, count_parameters # Assuming these are available or can be adapted
from cleanfid import fid # May need to be removed or adapted for MNIST
import torchvision.transforms.functional as TF
import matplotlib.pyplot as plt
from torchvision import transforms
from einops import rearrange
import json
from cfm_0707 import Cfm # Your custom CFM model
import math
import time
import random
...
config = {
    ...
    "output_dir": "./logs_fm_0708_mnist", # Changed output directory for MNIST
    "beta_1": 0.9,
    "beta_2": 0.99,
    'weight_decay': 0.001,
    "sigma_min": 1e-5,
...
model = Cfm(
    ...
    depth=cfg.depth,
    heads=cfg.num_heads,
    num_classes=10, # Changed num_classes to 10 for MNIST
    attn_dropout=0.0,
    ff_dropout=0.1
)
model.to(device)
...
# Transform 정의: MNIST에 맞게 변경
transform = transforms.Compose([
    transforms.Resize((64, 64)), # Resize MNIST to 64x64 to match VAE input expectations
    transforms.Grayscale(num_output_channels=3), # Convert to 3 channels (RGB)
    transforms.ToTensor(),
    # transforms.Normalize([0.5], [0.5]) # VAE typically expects input in [-1, 1] range, but your current code directly uses output from vae.decode in [0, 1]
])
...
val_dataset = HFDatasetWrapper(dataset["test"], transform=transform) # MNIST has 'test' set, not 'validation'
...
last_saved_epoch = 0
# For MNIST, we don't typically use labels_for_fid.json as FID is less common.
# If you need class-conditional generation, you'd generate labels programmatically.
# For now, let's just make a dummy labels list for sampling.
# with open('labels_for_fid.json', 'r') as f:
#     labels = json.load(f)
dummy_labels = [i % 10 for i in range(256 * 8)] # Generate 2048 labels (0-9 repeating) for FID sampling
# This is a placeholder, adapt if you have specific label requirements for FID.
...
def valid_step(epoch):
    ...
    with torch.no_grad():
        for idx, data in enumerate(valid_dataloader):
            ...
            b, c, h, w = z_0.shape
            seq_len = h * w # Define seq_len here for validation step as well
        ...
        if epoch % 1 == 0:
            ...
            # For MNIST, you might not use FID, or you'd generate a specific number of samples (e.g., 10,000)
            # and compare against the original MNIST test set.
            # Commenting out FID related code for now as it's specific to Imagenet paths.
            for cnt in tqdm(range(8)): # Generating 8 batches of 256 images = 2048 images
                y0 = torch.randn((256, 64, 4), device=device) # (batch_size, seq_len, latent_dim)
                ...
                # Use the dummy_labels for sampling classes
                label_class = torch.tensor(dummy_labels[256*cnt: 256*cnt + 256]).to(device)
                ...
                print(f"Generating samples for FID/visuals (batch {cnt+1}/8)")
                # ODE solve
                with torch.no_grad():
                    solution = odeint(ode_func, y0, t, method='rk4')  # shape: [64, b, seq_len, c]
            ...
            # get FID
            # You would typically calculate FID against the real MNIST test set.
            # For demonstration, I'm commenting this out.
            # fid_score = fid.compute_fid(
            #     "/workspace/personal_tests/valid_set/", # Path to your real MNIST test images (or a generated set)
            #     save_dir
            # )
    ...
    # if val_loss <= min(trainer['valid_losses']): # This condition might prevent saving early models
    ...
    # fid logging
    if fid_score is not None: # Only log if FID was calculated
