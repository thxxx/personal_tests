...
def valid_step(epoch):
    ...
    with torch.no_grad():
        for idx, data in enumerate(valid_dataloader):
            ...
            img_mask = torch.ones((b, seq_len//4)).to(device).bool() # patch_size * 2로 나누기
            ...
            if idx==0:
                ...
                img_mask = torch.ones((8, seq_len//4)).to(device).bool()
                ...
                # ODE solve
                with torch.no_grad():
                    solution = odeint(ode_func, y0, t, method='rk4')  # shape: [64, b, c, h, w]
        ...
        if epoch % 2 == 1:
            ...
            for cnt in tqdm(range(8)):
                ...
                img_mask = torch.ones((256, seq_len//4)).to(device).bool()
...
for epoch in range(cfg.epochs):
    ...
    for idx, data in enumerate(train_dataloader):
        ...
        context = torch.zeros_like(z_0)
        img_mask = torch.ones((b, seq_len//4)).to(device).bool()
